...
<div class="mb-6">
    ...
    <nav class="mt-4 flex flex-wrap gap-x-1 gap-y-2 border-b border-stone-200 pb-3 text-sm" aria-label="Coach sections">
        ...
        <a href="{{ route('coach.announcements.index', $tenant) }}" class="rounded-full px-3 py-1.5 hover:bg-stone-100 {{ request()->routeIs('coach.announcements.*') ? 'bg-amber-50 font-medium text-amber-950' : 'text-stone-600' }}">Announcements</a>
        ...
        <a href="{{ route('coach.course-reviews.index', $tenant) }}" class="rounded-full px-3 py-1.5 hover:bg-stone-100 {{ request()->routeIs('coach.course-reviews.*') ? 'bg-amber-50 font-medium text-amber-950' : 'text-stone-600' }}">Course reviews</a>
