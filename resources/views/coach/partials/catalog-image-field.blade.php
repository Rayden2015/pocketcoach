@php
    $imageUrl = isset($model) && method_exists($model, 'resolvedImageUrl') ? $model->resolvedImageUrl() : null;
    $label = $label ?? 'Cover image';
    $hint = $hint ?? 'JPG, PNG, GIF, or WebP — up to 10 MB. Shown on the catalog and course page.';
@endphp
<div>
    <label for="image" class="block text-sm font-medium text-stone-700">{{ $label }}</label>
